...
checkspeedtest() {
    if  [ ! -e '/tmp/speedtest.py' ]; then
        wget --no-check-certificate -P /tmp https://raw.githubusercontent.com/du5/speedtest-cli/master/speedtest.py > /dev/null 2>&1
    fi
...
speed_test(){
    ...
    true > $speedLog
        python /tmp/speedtest.py --search="China" --server $1 --share > $speedLog 2>&1
        is_upload=$(cat $speedLog | grep 'Upload')
        if [[ ${is_upload} ]]; then
            local REDownload=$(cat $speedLog | awk -F ' ' '/Download/{print $2}')
            local reupload=$(cat $speedLog | awk -F ' ' '/Upload/{print $2}')
            local relatency=$(cat $speedLog | awk -F ':' '/Hosted/{print $2}' | awk -F ' ' '{print $1}')
...
preinfo() {
    ...
    echo "       节点更新: 2020/05/25  | 脚本更新: 2020/05/25"
    echo "——————————————————————————————————————————————————————————"
}
...
runtest() {
    ...
    if [[ ${selection} == 1 ]]; then
        ...
        #***
         speed_test '5530' '重庆' '广电'
         speed_test '30852' '江苏昆山杜克大学' '教育'
